...
export const ElectricityScreen = ({ navigation }) => {
  const [electirictyPhotos, updateElectricityPhotosArray] = useState([]);
  ...
  const loadPhotosElectricity = async () => {
    try {
      const value = await AsyncStorage.getItem("photo-object-e");
      if (value !== null) {
        console.log("valuedetected");
        updateElectricityPhotosArray(JSON.parse(value));
      }

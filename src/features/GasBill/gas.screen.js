...
export const GasScreen = ({ navigation }) => {
  const [gasPhotos, updateGasPhotosArray] = useState([]);
  ...
  const loadPhotosGas = async () => {
    try {
      const value = await AsyncStorage.getItem("photo-object-d");
      if (value !== null) {
        console.log("valuedetected");
        updateGasPhotosArray(JSON.parse(value));
      }
